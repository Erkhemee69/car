...
import {
  TrendingUp, Car, Clock, Settings, Save, RefreshCw, List
} from "lucide-react";
import {
  useGetAdminStats,
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'dashboard' && (
          <motion.div key="dashboard" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
            <div className="flex justify-end">
              <button onClick={() => refetchStats()} className="flex items-center gap-2 text-sm text-primary hover:text-primary/80 transition-colors">
            ...
            <div className="glass-panel rounded-3xl p-6 border border-white/5">
              <h3 className="text-lg font-semibold text-white mb-6">Сүүлийн үеийн орлого (₮)</h3>
                ...
                {chartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData}>
                      ...
                      <YAxis stroke="#888888" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(v) => v >= 1000 ? `${v / 1000}k` : v} />
                      <Tooltip cursor={{ fill: '#ffffff05' }} contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '12px', color: '#fff' }} />
        ...
        {activeTab === 'records' && (
          <motion.div key="records" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="glass-panel rounded-3xl border border-white/5 overflow-hidden">
          ...
          <motion.div key="settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="glass-panel rounded-3xl p-8 border border-white/5">
              ...
              <form onSubmit={handleSaveTariff} className="space-y-6">
                ...
                <button type="submit" disabled={updateTariff.isPending} className="w-full py-4 rounded-xl bg-primary hover:bg-primary/90 text-white font-medium flex items-center justify-center gap-2 transition-all disabled:opacity-50">
                  <Save className="w-4 h-4" /> {updateTariff.isPending ? "Хадгалж байна..." : "Хадгалах"}
                </button>
              </form>
            </div>
            ...
            <div className="glass-panel rounded-3xl p-8 border border-white/5">
              ...
              <form onSubmit={handleSaveCapacity} className="space-y-6">
                ...
                <button type="submit" disabled={updateCapacity.isPending} className="w-full py-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-medium flex items-center justify-center gap-2 transition-all disabled:opacity-50">
                  <Save className="w-4 h-4" /> {updateCapacity.isPending ? "Хадгалж байна..." : "Хадгалах"}
                </button>
              </form>
            </div>
          </motion.div>
